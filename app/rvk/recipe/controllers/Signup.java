...
import rvk.recipe.models.TokenAction;
import rvk.recipe.models.TokenAction.Type;
import rvk.recipe.models.User;
import rvk.recipe.providers.MyLoginUsernamePasswordAuthUser;
import rvk.recipe.providers.MyUsernamePasswordAuthProvider;
import rvk.recipe.providers.MyUsernamePasswordAuthProvider.MyIdentity;
import rvk.recipe.providers.MyUsernamePasswordAuthUser;
	...
	public static Result doResetPassword() {
		...
		if ( filledForm.hasErrors() ) {
			...
			try {
				...
				flash( Application.FLASH_MESSAGE_KEY, Messages.get( "playauthenticate.reset_password.message.no_password_account" ) );
			}
			final boolean login = MyUsernamePasswordAuthProvider.getProvider().isLoginAfterPasswordReset();
			if ( login ) {
				// automatically log in
				flash( Application.FLASH_MESSAGE_KEY, Messages.get( "playauthenticate.reset_password.message.success.auto_login" ) );
				return PlayAuthenticate.loginAndRedirect( ctx(), new MyLoginUsernamePasswordAuthUser( u.email ) );
			}
			else {
				// send the user to the login page
				flash( Application.FLASH_MESSAGE_KEY, Messages.get( "playauthenticate.reset_password.message.success.manual_login" ) );
			}
